...
document.addEventListener('DOMContentLoaded', () => {
	// Insert the product HTML into the grid
	...
	function updateCartQuantity() {
		...
		// Get the cart from localStorage and parse it (if it exists)
		const savedCart = JSON.parse(localStorage.getItem('cart'));
		if (savedCart) {
			savedCart.forEach((cartItem) => {
				cartQuantity += cartItem.quantity; // Sum up the quantities
			});
		}
	...
	// Update the cart quantity when the page loads (restore saved cart)
	updateCartQuantity();
